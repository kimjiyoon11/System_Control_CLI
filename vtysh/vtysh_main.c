...
static void usage (char *progname, int status)
{
	printf ("Usage : %s [OPTION...]\n"
		...
		"\t-c, --config        Load the config file,default["CONFIG_DIR"/vtysh.conf]\n"
...
static void in_show_welcome()
{
	...
	sprintf(build, "Build On Tue Sep 14 13:49:39 CST 2010");
	fp = fopen("/BUILD", "r");
	if(fp)
	{
	...
	vty_out(vty, "********************************************************************************\n");
	vty_out(vty, "*  Copyright(c) 2007-2010 Hangzhou Infogo Tech. Co., Ltd. All rights reserved. *\n");
	vty_out(vty, "*  Without the owner's prior written consent,                                  *\n");
	vty_out(vty, "*  no decompiling or reverse-engineering shall be allowed.                     *\n");
	vty_out(vty, "*                                                                              *\n");
	vty_out(vty, "*  %-40s                                    *\n", build);
	vty_out(vty, "********************************************************************************\n");
...
int main (int argc, char **argv, char **env)
{
	...
	int boot_flag = 0;
	char *eval_line = NULL;
	char *config_file = CONFIG_DIR "/vtysh.conf";
